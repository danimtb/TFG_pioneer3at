...
Por último, es necesario conocer algunos tipos de archivos y su uso dentro de ROS para comprender el trabajo de este proyecto:
...
Para el desarrollo de este proyecto es necesario clonar el repositorio GitHub \hypersetup{urlcolor=black} \href{https://github.com/danimtb/pioneer3at_ETSIDI}{\textit{ pioneeer3at\_ETSIDI}} \hypersetup{urlcolor=blue}en nuestra carpeta \textit{$\sim$/catkin\_ws/src}:
...
Junto con el repositorio de desarrollo también es preciso instalar las siguientes funcionalidades adicionales de ROS y gestionar algunas dependencias de librerías, para lo cual nos serviremos de la herramienta rosdep **referencia** creada por los desarrolladores de ROS para este propósito.

Inicializamos rosdep y actualizamos las pedendencias:
...
\end{lstlisting}
\hypersetup{urlcolor=black}
\caption{Inicializando la heamienta \textit{rosdep}.}
\end{code}

\begin{itemize}
\item Navegación: Instalamos los paquetes adicionales para realizar la navegación (se hablará de cada uno en el capítulo \ref{chapter:navegacion}).
\item Rosaria
\item pocketsphinx
\item rosdeps
\item mirar cuaderno
