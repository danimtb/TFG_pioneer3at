...
Pueden precisarse algunos ajustes previos con la herramienta que ofrece el fabricante "SOPAS Engineering tool", los cuales pueden encontrarse en el apéndice de este trabajo (Sección \ref{subsection:sicklms100}).
...
{\bf Parámetros} & {\bf Tipo} & {\bf Descripción}\\
\hline
min\_y & double & Minimum y position of points\\
max\_y & double & Maximum y position of points\\
min\_x & double & Minimum x position of points\\
max\_x & double & Maximum x position of points\\
max\_z & double & Maximum z position of points\\
goal\_z & double & Distance away from the robot\\
z\_scale & double & Scaling factor for trans. speed\\
x\_scale & double & Scaling factor for rot. speed\\
enabled & bool & Enable/disable movement commands\\
\end{tabular}
}
...
\hypersetup{urlcolor=blue}
\caption{Launchfile para turtlebot\_follower.}
\end{code}
