...
En ROS el espacio de trabajo es el lugar donde se realiza el desarrollo software de los paquetes ROS. Este entorno de trabajo es capaz de gestionar la correcta compilación del código de los nodos.
...
A continuación se describen los pasos para crear un espacio de trabajo Catkin (extraídos del tutorial \url{http://wiki.ros.org/catkin/Tutorials/create_a_workspace}) y los pasos para clonar el repositorio de GitHub que aloja el código.
...
Sin embargo, es recomendable que si se desea realizar algún desarrollo posterior, se realice un \textit{fork} en GitHub de este proyecto (Figura \ref{fig:pioneer3at_ETDSIDI}) y se clone el repositorio propio.
...
De esta forma podemos guardar los cambios realizados en el repositorio de la persona que hace la modificación, con la intención de incorporar los cambios más tarde al repositorio de desarrollo principal (\url{https://github.com/danimtb/pioneer3at_ETSIDI.git}) mediante \textit{Pull Request}.

\subsection{Gestión de las dependencias}\label{subsection:dependencias}

 ROS se vale de la información almacenada en la definición de cada uno de sus paquetes en \textit{package.xml} y gestionar algunas dependencias de librerías, para lo cual se sirve de la herramienta \textit{rosdep} creada por los desarrolladores de ROS para este propósito.
...
Es posible que necesitemos las dependencias añadidas de \textit{audio\_capture}.
...
\begin{itemize}
\item TicksMM: Pulsos por cada milímetro.
\item DriftFactor: Valor de pulsos para compensar las diferencias entre las ruedas de un lado y del otro.
\item RevCount: Número de pulsos en los encoders para que el robot realice un giro de 180º.
\end{itemize}

Del propio manual del robot extraemos la siguiente información:
\begin{itemize}
\item Pulsos por revolución (ticks/rev): 500.
\item Relación de transmisión (gear ratio): 49'8.
\end{itemize}

Para ajustar \textit{TicksMM} utilizamos la fórmula del manual:
...
Para ajustar \textit{RevCount} situamos al robot paralelo a una referencia en el suelo (una línea recta en el suelo), marcamos el punto en el que las ruedas tocan el suelo y manualmente, agarrando al robot por las ruedas, giramos las mismas hasta que el robot quede girado 180º. Contamos las revoluciones de una de las ruedas gracias a las marcas que habíamos hecho y hacemos el siguiente cálculo:
...
Por último, para ajustar \textit{DriftFactor} procedemos de manera experimental. Situamos al robot pararelo a una linea recta marcada en el suelo y lo teleoperamos hacia adelante unos 4 metros. Si observamos que el robot se separa de la linea recta, ajustamos el valor de este parámetro en consecuencia.

Para más información, se recomienda leer el manual del fabricante \cite{pioneer2006}.

\subsection{Ordenador de a bordo Intel NUC}
Los datos de incio de sesión del ordenador son:
\begin{itemize}
\item Usuario: pioneer3at.
\item Contraseña: pioneer3at.
...
El nodo máster de ROS se ejecuta en este ordenador, por lo que es preciso indicarlo en el archivo \textit{.bashrc}.
...
El sensor Kinect utilizado en el proyecto no dispone de ninguna modificación específica o modo de funcionamiento especial. Lo referente a su conexionado ha sido descrito anteriormente en la sección \ref{subsection:implementacion_kinect}.

Tan solo indicar que puede realizarse una primera toma de contacto a través de la librería \textit{libfreenect}.

Una manera rápida de hacerlo es utilizando el gestor de depencencias en C++ biicode (\url{www.biicode.com}), en un sistema operativo Ubuntu siguiendo esta guía \url{http://blog.biicode.com/kinect-xbox360-biicode/}.

\subsection{Láser SICK LMS100}\label{subsection:sicklms100_apendice}

El sensor láser LMS100 de la marca Sick es un sensor de amplio rango y largo alcance de tipo industrial.
...
\begin{itemize}
\item Dirección IP del láser Sick LMS100: 10.42.1.197
\item Dirección IP del adaptador ethernet del ordenador Intel NUC: 10.42.1.1
\end{itemize}
...
Adicionalmente es necesario encender el ordenador de a bordo Intel NUC mediante su correspondiente botón y el altavoz frontal del robot, para lo cual será necesario tener activada la alimentación auxiliar 2 (AUX 2) en el panel de control.
...
\caption{Botón de parada de emergencia del robot Petrois.}
...
Para rearmar el robot basta con devolver la seta de emergencia a su posición DE reposo girando levemente la misma en el sentido de las flechas. Después es necesario pulsar el botón MOTORS para volver a habilitar los motores.
...
Para información más extensa y detallada se recomienda leer la guía de usuario ofrecida por el fabricante \cite{pioneer2006}.

\section{Conexión mediante un ordenador externo vía Wifi}\label{section:ordenador_externo}
...
export ROS_MASTER_URI=http://10.42.0.1:11311
\end{lstlisting}
\caption{Añadiendo las direcciones IP al \textit{.bashrc}.}
\end{code}
...
Este procedimiento está hecho igual en el Intel NUC pero indicando tan solo que la IP de ese equipo es \textbf{ROS\_IP=10.42.0.1}.
...
\section{Acceder a la placa de alimentación}\label{section:placa_alimentacion}
...
\caption{Trampilla de acceso a las baterías.}
\label{fig:baterias}
\end{figure}

La carga de las baterías se realiza con el adaptador de la figura \ref{fig:cargador} a través del conector de carga situado junto al interruptor de alimentación general.
...
En él se encuentra una pequeña guía \textit{Readme} actualizada con información sobre las utilidades que ofrece el repositorio y en concreto el desarrollo realizado en el paquete \textit{pioneer\_utils}.
...
Colección de vídeos:
\begin{itemize}
\item Algunos vídeos de este proyecto:
\begin{itemize}
